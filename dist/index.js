...
server.tool('download_video_no_watermark', 'Download a TikTok video without watermark. Returns HD video URL ready for download. Example: download_video_no_watermark("https://www.tiktok.com/@user/video/1234567890")', {
    tiktok_url: zod_1.z.string().url().describe('Full TikTok video URL, e.g. https://www.tiktok.com/@username/video/1234567890')
}, async ({ tiktok_url }) => {
...
server.tool('get_video_info', 'Get full metadata for a TikTok video including stats, author info, and music. Example: get_video_info("https://www.tiktok.com/@charlidamelio/video/1234567890")', {
    tiktok_url: zod_1.z.string().url().describe('Full TikTok video URL')
}, async ({ tiktok_url }) => {
    const data = await videoApiFetch(tiktok_url);
    const result = {
        title: data.title || null,
...
server.tool('get_user_profile', 'Get a TikTok user\'s profile information including follower count, bio, and verification status. Example: get_user_profile("charlidamelio")', {
    username: zod_1.z.string().min(1).describe('TikTok username without @ symbol, e.g. "charlidamelio"')
}, async ({ username }) => {
...
server.tool('get_user_videos', 'Get recent videos from a TikTok user\'s profile. Returns video list with stats and download URLs. Example: get_user_videos("khaby.lame", 20)', {
    username: zod_1.z.string().min(1).describe('TikTok username without @ symbol'),
    count: zod_1.z.number().int().min(1).max(100).default(20).describe('Number of videos to fetch (1-100, default 20)'),
    cursor: zod_1.z.string().optional().describe('Pagination cursor from previous response for loading more videos')
...
server.tool('get_user_followers', 'Get follower list for a TikTok user. Example: get_user_followers("mrbeast", 50)', {
    username: zod_1.z.string().min(1).describe('TikTok username without @ symbol'),
    count: zod_1.z.number().int().min(1).max(200).default(50).describe('Number of followers to fetch (1-200, default 50)'),
    min_cursor: zod_1.z.string().optional().describe('Pagination cursor for next page of followers')
...
server.tool('get_user_following', 'Get accounts that a TikTok user is following. Example: get_user_following("addisonre", 50)', {
    username: zod_1.z.string().min(1).describe('TikTok username without @ symbol'),
    count: zod_1.z.number().int().min(1).max(200).default(50).describe('Number of following accounts to fetch (1-200, default 50)'),
    min_cursor: zod_1.z.string().optional().describe('Pagination cursor for next page')
}, async ({ username, count, min_cursor }) => {
    const params = { unique_id: username, count };
    if (min_cursor)
        params['min_cursor'] = min_cursor;
...
server.tool('get_trending_videos', 'Get currently trending TikTok videos. Returns top videos with engagement stats. Example: get_trending_videos("US", 20)', {
    region: zod_1.z.string().length(2).default('US').describe('Two-letter country code for regional trends (US, GB, AU, etc.)'),
    count: zod_1.z.number().int().min(1).max(50).default(20).describe('Number of trending videos (1-50, default 20)')
}, async ({ region, count }) => {
...
server.tool('get_trending_hashtags', 'Get currently trending hashtags on TikTok. Example: get_trending_hashtags("US")', {
    region: zod_1.z.string().length(2).default('US').describe('Two-letter country code (US, GB, CA, AU, etc.)')
}, async ({ region }) => {
...
server.tool('get_trending_sounds', 'Get currently trending sounds and music on TikTok. Useful for identifying audio trends for content creation. Example: get_trending_sounds("US")', {
    region: zod_1.z.string().length(2).default('US').describe('Two-letter country code (US, GB, CA, etc.)')
}, async ({ region }) => {
...
server.tool('search_videos', 'Search TikTok for videos matching a keyword or phrase. Example: search_videos("morning routine", 20)', {
    query: zod_1.z.string().min(1).describe('Search query, e.g. "morning routine", "cooking tips", "#fitness"'),
    count: zod_1.z.number().int().min(1).max(50).default(20).describe('Number of results (1-50, default 20)'),
    cursor: zod_1.z.string().optional().describe('Pagination cursor for next page of results')
...
server.tool('search_users', 'Search TikTok for users matching a name or keyword. Example: search_users("fitness coach", 20)', {
    query: zod_1.z.string().min(1).describe('Search query for finding TikTok users, e.g. "fitness coach", "cooking", "tech reviewer"'),
    count: zod_1.z.number().int().min(1).max(50).default(20).describe('Number of results (1-50, default 20)'),
...
server.tool('search_hashtags', 'Search TikTok for hashtags matching a keyword. Find related hashtags for content strategy. Example: search_hashtags("fitness")', {
    query: zod_1.z.string().min(1).describe('Hashtag search term (with or without #), e.g. "fitness", "cooking", "travel"'),
    count: zod_1.z.number().int().min(1).max(50).default(20).describe('Number of results (1-50, default 20)')
}, async ({ query, count }) => {
...
server.tool('get_video_comments', 'Get comments on a TikTok video. Example: get_video_comments("7123456789012345678", 20)', {
    video_id: zod_1.z.string().min(1).describe('TikTok video ID (numeric string found in video URL)'),
    count: zod_1.z.number().int().min(1).max(100).default(20).describe('Number of comments to fetch (1-100, default 20)'),
    cursor: zod_1.z.string().optional().describe('Pagination cursor for next page of comments')
...
server.tool('get_video_likes', 'Get users who liked a TikTok video. Example: get_video_likes("7123456789012345678", 50)', {
    video_id: zod_1.z.string().min(1).describe('TikTok video ID (numeric string from video URL)'),
    count: zod_1.z.number().int().min(1).max(100).default(50).describe('Number of likers to fetch (1-100, default 50)'),
    cursor: zod_1.z.string().optional().describe('Pagination cursor for next page')
}, async ({ video_id, count, cursor }) => {
    const params = { video_id, count };
    if (cursor)
...
server.tool('get_video_shares', 'Get share count and engagement stats for a TikTok video by URL. Example: get_video_shares("https://www.tiktok.com/@user/video/7123456789012345678")', {
    tiktok_url: zod_1.z.string().url().describe('Full TikTok video URL')
}, async ({ tiktok_url }) => {
    const data = await videoApiFetch(tiktok_url);
    const result = {
...
server.tool('get_creator_analytics', 'Get comprehensive analytics for a TikTok creator including engagement rates, average views, and top videos. Example: get_creator_analytics("garyvee")', {
    username: zod_1.z.string().min(1).describe('TikTok username without @ symbol'),
    video_sample_size: zod_1.z.number().int().min(5).max(50).default(20).describe('Number of recent videos to analyze (5-50, default 20)')
    ...
    const topVideos = [...videos]
        .sort((a, b) => (Number(b.stats?.playCount) || 0) - (Number(a.stats?.playCount) || 0))
        .slice(0, 5)
        .map((v) => ({
        title: v.desc || null,
        views: formatCount(v.stats?.playCount),
...
server.tool('get_audience_insights', 'Get audience demographic insights for a TikTok creator based on follower analysis and engagement patterns. Example: get_audience_insights("charlidamelio")', {
    username: zod_1.z.string().min(1).describe('TikTok username without @ symbol'),
    follower_sample: zod_1.z.number().int().min(10).max(100).default(50).describe('Number of followers to sample for demographic analysis (10-100, default 50)')
    ...
    const followerFollowerCounts = followers.map((f) => Number(f.followerCount) || 0).filter(c => c > 0);
